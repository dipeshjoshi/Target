...
process_item <- function(datain, curr_key)
{
	...
	if(train_series != "error"){
		...
		if(item_model[[2]] != "arima"){
			...
			save_backup_model(model_path, item_id, rel_date, backup_model[[1]], backup_model[[2]])
		}
		
		if(item_model[[1]] != "error"){

			save_model(model_path, item_id, rel_date, item_model[[1]], item_model[[2]])
			#save_model_type(model_path, item_id, rel_date, item_model[[2]])
...
save_model_type <- function(directory,unique_id, rel_date, model_type)
{
        save(model_type, file = paste('mt_',unique_id,'.rda', sep = ''))
        system(sprintf("hdfs dfs -rm -f %s/%s/mt_%s.rda", directory, rel_date, unique_id))
        system(sprintf("hdfs dfs -put mt_%s.rda %s/%s/", unique_id, directory, rel_date))

}

save_backup_model <- function(directory,unique_id, rel_date, model_object, model_type)
{
...
save_model <- function(directory,unique_id, rel_date, model_object, model_type)
{
        save(model_object, file = paste(unique_id,'.rda', sep = ''))
	mapping[nrow(mapping)+1, ] <<- c(as.integer(unique_id), as.character(compressed_file), as.character(model_type))
	
	if(comp_flag == 0){
		print("MODEL 1st ITEM")
		system(sprintf("tar -cvf %s %s.rda", compressed_file, unique_id))
		comp_flag <<- 1
	}
	else{
		print("MODEL NOT A 1st ITEM")
		system(sprintf("tar -uvf %s %s.rda", compressed_file, unique_id))
	}
}


create_directory <- function(directory, rel_date)
{
        system(sprintf("hdfs dfs -mkdir -p %s/%s/", directory, rel_date))
}


create_directory(model_path, rel_date)
...
if(!no_item_flag){
	system(sprintf("hdfs dfs -put %s %s/%s/", compressed_file, model_path, rel_date))
	...
	system(sprintf("hdfs dfs -put %s %s/%s/", mapping_file, model_path, rel_date))
}
