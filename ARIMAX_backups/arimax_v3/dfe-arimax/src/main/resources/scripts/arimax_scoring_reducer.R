...
# model release date
rel_date = as.character(args[2])
# path to the model directory
model_path = as.character(args[3])
...
log_df <- data.frame(item_id=integer(), model_type=character(), stringsAsFactors=FALSE)



load_backup_model <- function(directory, unique_id, rel_date)
{
         model<-tryCatch(
                {
			compressed_file_name <- mapping[mapping[ ,"V1"] == as.character(unique_id), 2][1]
			system(sprintf("hdfs dfs -copyToLocal %s/%s/%s $(pwd)/", model_path, rel_date, compressed_file_name))    
...
load_model <- function(model_path, unique_id, rel_date)
{
	model<-tryCatch(
		{
			compressed_file_name <- mapping[mapping[ ,"V1"] == as.character(unique_id), 2][1]		
...
load_model_type <- function(directory, unique_id, rel_date)
{
        type<-tryCatch(
                {
...
{	
	log_df[nrow(log_df)+1, ] <<- c(as.integer(item_id), as.character(model_type))
	...
	if(model_type == "sarima"){
        ...
        if(fcst == "error"){
		log_df <<- log_df[-nrow(log_df), ]
		log_df[nrow(log_df)+1, ] <<- c(as.integer(item_id), as.character("arima"))
		fit <- load_backup_model(model_path, item_id, rel_date)
...
process_item <- function(datain, curr_key)
{
	...
	model_object <- load_model(model_path, item_id, rel_date)
	model_type <- load_model_type(model_path, item_id, rel_date)
	...
	if(model_object != "error" & model_type !="error")
	{
		...
		{
			if(is.null(item_forecast[i])){
                                cat(item_id,"\t",datain$dept_id[1],"\t",as.character(datain_ordered$week_end_date[ i]),"\t",item_forecast[i],"\n")
                        }else if(is.na(item_forecast[i])){
                                cat(item_id,"\t",datain$dept_id[1],"\t",as.character(datain_ordered$week_end_date[ i]),"\t",item_forecast[i],"\n")
                        }else if(item_forecast[i] < 0)
                        {       
                                cat(item_id,"\t",datain$dept_id[1],"\t",as.character(datain_ordered$week_end_date[ i]),"\t","0","\n")
                        }else   
                        {       
                                cat(item_id,"\t",datain$dept_id[1],"\t",as.character(datain_ordered$week_end_date[ i]),"\t",item_forecast[i],"\n")
                        }
...
#Read Mapping file 
mapping_file_path <- paste(model_path,'/',rel_date ,'/mapping.csv', sep='')
...
log_file <- paste(file_name, '_log.txt', sep="")
no_item_flag <- TRUE
...
if(!no_item_flag){
	write.table(log_df, file = log_file, row.names = FALSE, col.names = FALSE, sep = ',')
	system(sprintf("hdfs dfs -put %s %s/%s/", log_file, model_path, rel_date))
}
