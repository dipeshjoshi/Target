...
drop table if exists ${hivevar:ARIMAX_DB}.${hivevar:SCORE_INPUT_TABLE};
create table ${hivevar:ARIMAX_DB}.${hivevar:SCORE_INPUT_TABLE} as select t1.mdse_item_i, t1.mdse_clas_i, t1.mdse_dept_ref_i, t1.week_start_date, t1.week_end_date as week_end_date, 0, retl_a, 0, 0 as avg_sales, n_stores, circular_flag, circular_flag_count, 0, 0, dollar_off_flag, dollar_off_flag_count, tpc_flag, tpc_flag_count, pct_off_flag, pct_off_flag_count,
...
from ${hivevar:SALES_FORECAST_TABLE} t1 LEFT OUTER JOIN ${hivevar:CALENDAR_BTS_TABLE} t2 on t1.week_end_date=t2.week_end_date 
left outer join ${hivevar:CALENDAR_CHRISTMAS_TABLE} t3 on t1.week_end_date=t3.week_end_date inner join (select distinct mdse_item_i from prd_dfe_slshist.master_item_mapping where item_type <> 'WEB_ONLY' and new_item = 'N' and mdse_item_i is not NULL) c on (t1.mdse_item_i = c.mdse_item_i) where t1.mdse_item_i is not NULL;
